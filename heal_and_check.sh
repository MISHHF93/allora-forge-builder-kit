...
SCRIPT_PATH=".venv/bin/python submit_prediction.py --continuous"
...
if [ -n "$pid" ]; then
    ...
    echo "[ACTION] Restarting daemon..."
    nohup $SCRIPT_PATH > "$LOG_FILE" 2>&1 &
    sleep 2
    new_pid=$(pgrep -f submit_prediction.py)
    if [ -n "$new_pid" ]; then
        echo "✅ Restarted successfully (New PID: $new_pid)"
    else
...
echo "[CHECK COMPLETE] System and pipeline are healthy (restarted if needed)."
